...
- FTS5 queries sanitized via `sanitizeFtsInput()` before execution
...
- Source data from official Justel/Belgisch Staatsblad (auditable)
- Ingestion scripts require manual execution (no auto-download at runtime)
- Journal mode set to DELETE (not WAL) for Vercel serverless compatibility
