/**
 * search_eu_implementations — Search EU directives/regulations with Belgian implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_belgian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS belgian_statute_count,
  ...
  if (input.has_belgian_implementation !== undefined) {
    sql += input.has_belgian_implementation ? ` HAVING belgian_statute_count > 0` : ` HAVING belgian_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    belgian_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        belgian_statute_count: r.belgian_statute_count,
